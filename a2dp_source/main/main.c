...
// were just going to do 441 Hz
#define TESTHZ 441
#define SAMPLE_RATE 44100
#define AUDIO_BUFSIZE 100
#define AMPLITUDE 8192.0
int16_t audioBuffer[AUDIO_BUFSIZE][2];
...
void printMyMac(void)
{
	...
	blah = esp_bt_dev_get_address();
	if (blah) {
		memcpy(mac, esp_bt_dev_get_address(), 6);
    	ESP_LOGI(BT_AV_TAG, "My MAC Address: %s", bda2str(mac, bda_str, 18));
	} else {
    	ESP_LOGI(BT_AV_TAG, "Mac is null");
	}
...
void app_main()
{
	...
	for (int i=0; i<AUDIO_BUFSIZE; i++) {
		audioBuffer[i][0] = AMPLITUDE * sinf(TESTHZ * i * 2 * M_PI / SAMPLE_RATE);
		audioBuffer[i][1] = AMPLITUDE * sinf(TESTHZ*2.0 * i * 2 * M_PI / SAMPLE_RATE);
	}
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    ESP_LOGI(BT_AV_TAG, "cbCount0: %d", cbCount0);
    ESP_LOGI(BT_AV_TAG, "cbCount1: %d", cbCount1);
...
static int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len)
{
	...
	int16_t* myDataPointer = (int16_t *)data;
    for (int i = 0; i < (len >> 1); i+=2) {
		myDataPointer[i] = audioBuffer[audioBufferIndex][0];
		myDataPointer[i + 1] = audioBuffer[audioBufferIndex][1];
		if (++audioBufferIndex >= AUDIO_BUFSIZE)
			audioBufferIndex = 0;
	}
#else
    int val = rand() % (1 << 16);
